...
export type SettingsScreen = "teams" | "team" | "agent" | "logs" | "history" | "nexus" | "jova" | "documents" | "chat" | "voice";
...
interface SettingsState {
  ...
  teamId: string | null;
  /** agent in focus for the agent screen */
  agentId: string | null;
...
export const useSettingsStore = create<SettingsState>((set) => ({
  ...
  showDocuments: () => set({ screen: "documents" }),
  ...
  back: () =>
    set((st) =>
      st.screen === "agent" ? { screen: "team" } : st.screen === "team" ? { screen: "teams" } : { open: false }
    ),
}));
